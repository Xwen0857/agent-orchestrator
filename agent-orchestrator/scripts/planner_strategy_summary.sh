...
load_planner_strategy_summary() {
  ...
  SUMMARY_CONSTRAINTS="$(jq -r '((.summary_input.constraints // []) | map(select(type == "string" and length > 0)) | join("; "))' "$strategy_path")"
  SUMMARY_DELIVERABLES="$(jq -r '((.summary_input.deliverables // []) | map(select(type == "string" and length > 0)) | join("; "))' "$strategy_path")"
  SUMMARY_NOTES="$(jq -r '((.summary_input.notes // []) | map(select(type == "string" and length > 0)) | join("; "))' "$strategy_path")"
